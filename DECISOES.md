...
- **No banco de dados:** a regra evoluiu em duas etapas. Primeiro adicionei uma restrição única combinando tipo, data e horário  mas logo percebi que isso só bloqueava o horário exatamente igual (ex: duas consultas médicas às 09:30), e não cobria dois agendamentos muito próximos (ex: 09:30 e 09:31), que na prática também são inviáveis, porque uma consulta não é instantânea. Troquei pela solução final: uma restrição de exclusão (`EXCLUDE`, com a extensão `btree_gist`) que impede dois agendamentos do mesmo tipo com menos de 30 minutos de diferença entre si, usando um intervalo de tempo (`tsrange`) em vez de uma igualdade simples. Errei na primeira versão dessa regra também: coloquei só data e horário, sem o tipo, o que teria bloqueado, por exemplo, uma consulta médica e uma vacinação marcadas no mesmo horário  mas isso não é um choque de verdade, porque são atendidos por pessoas diferentes. A versão final já nasceu com o tipo incluído.
- **No formulário:** quando essa restrição barra o envio, o visitante vê uma mensagem específica explicando que o horário está muito próximo de outro já existente para aquele tipo de atendimento, com o intervalo mínimo exigido.

Também percebi, ao revisar o fluxo, que cada tipo de atendimento tem dias e horários de funcionamento próprios na realidade de uma ESF (ex: puericultura só terça e quinta, visita domiciliar só sexta). Adicionei essa informação na página pública (em cada serviço) e uma validação no formulário que bloqueia dia/horário fora da janela de cada tipo, com uma mensagem explicando o horário correto.

Diferente da regra de choque de horário, decidi **não** duplicar essa validação de dias/horários de funcionamento no banco de dados. A regra de choque protege contra uma condição de corrida real (dois envios simultâneos criando um conflito que nenhum dos dois lados sozinho detectaria)  por isso precisa estar no banco. Já a regra de horário de funcionamento é uma regra de negócio, não uma questão de integridade de dados concorrentes: validar no formulário já cobre o caso de uso real do sistema (o visitante usando o site normalmente), e duplicar cada janela de horário por tipo como `CHECK constraint` no banco aumentaria bastante a complexidade do schema para um ganho pequeno nesse projeto. Se o sistema fosse para produção de verdade, replicaria essa validação no banco também.
