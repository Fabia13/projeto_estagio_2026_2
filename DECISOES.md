...
Depois de enviar o Pull Request, percebi uma falha que não tinha tratado: nada impedia que dois visitantes agendassem a mesma data e o mesmo horário  geraria um "choque" de agendamento. Corrigi em duas camadas, no mesmo espírito da proteção do painel:

- **No banco de dados:** adicionei uma restrição única em `sql/schema.sql` que impede fisicamente dois registros não cancelados com a mesma data e horário, mesmo que dois formulários sejam enviados ao mesmo tempo (cancelados não contam, para liberar o horário de novo).
- **No formulário:** quando essa restrição barra o envio, o visitante agora vê uma mensagem específica ("Esse horário já está reservado. Escolha outra data ou horário.") em vez do erro genérico.
